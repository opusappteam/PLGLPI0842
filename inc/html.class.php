...
/*
 * @version $Id: html.class.php 21754 2013-09-12 08:10:32Z tsmr $
  -------------------------------------------------------------------------
...
class Html {
    ...
    static function timestampToString($time, $display_sec = true, $use_days = true) {
        ...
        if ($use_days) {
            if ($units['day'] > 0) {
                if ($display_sec) {
                    ...
                    return $out . sprintf(__('%1$d days %2$d hours %3$d minutes %4$d seconds'), $units['day'], $units['hour'], $units['minute'], $units['second']);
                }
                //TRANS: %1$d number of days, %2$d number of hours,   %3$d number of minutes
                return $out . sprintf(__('%1$d days %2$d hours %3$d minutes'), $units['day'], $units['hour'], $units['minute']);
            }
        } else {
            if ($units['day'] > 0) {
        ...
        if ($units['hour'] > 0) {
            if ($display_sec) {
                ...
                return $out . sprintf(__('%1$d hours %2$d minutes %3$d seconds'), $units['hour'], $units['minute'], $units['second']);
            }
    ...
    static function includeHeader($title = '') {
        ...
        echo "\n<html><head><title>ITSM - " . $title . "</title>";
...
//             $menu['utils']['content']['report']['page'] = 'http://192.168.35.12/Reports/Pages/Folder.aspx?ItemPath=%2fTestSSRS&ViewMode=List';
        }
        ...
        echo "<div id='banner'><div id='c_logo'>";
        echo "<a href='" . $CFG_GLPI["root_doc"] . "/front/central.php' accesskey='1' title=\"" .
        __s('Home') . "\">";
        echo "<img src='../pics/lenovo-logo.gif' style='margin:20px;width:200px;'/></a></div></div>";
        ...
        foreach ($menu as $part => $data) {
            if (isset($data['content']) && count($data['content'])) {
                ...
                if ($part == 'utils') {
                    echo '<li><a href="http://192.168.202.228/Reports/Pages/Folder.aspx?ItemPath=%2fLenovoServiceCenterReport" target="_blank">Lenovo Reports</a></li>';
                }
...
//             echo "</script>";
                ...
                Ajax::createModalWindow('massiveaction_window' . $identifier, $url, array('title' => _n('Action', 'Actions', 2),
    ...
    static function showProfileSelecter($target) {
        ...
        if (Session::isMultiEntitiesMode()) {
            ...
            Ajax::createModalWindow('entity_window', $CFG_GLPI['root_doc'] . "/ajax/entitytree.php", array('title' => __('Select the desired entity'),
